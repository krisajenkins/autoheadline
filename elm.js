...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $UI = Elm.UI.make(_elm);
   var uiMailbox = $Signal.mailbox($Schema.NoOp);
   var initialModel = {_: {}
                      ,graph: $Maybe.Nothing
                      ,newsItems: $Http.NotAsked
                      ,phrase: _L.fromArray([$Markov.startToken])};
   var graphFromNews = function (r) {
      return function () {
         switch (r.ctor)
         {case "Success":
            return $Maybe.Just($Markov.createGraph(A2($List.map,
              function (_) {
                 return _.title;
              },
              r._0)));}
   ...
   model) {
      return function () {
         ...
         {case "ChooseToken":
            return function () {
                 var currentPhrase = model.phrase;
                 return _U.replace([["phrase"
                                    ,A2($Basics._op["++"],
                                    currentPhrase,
            ...
            case "LoadNews":
            return _U.replace([["graph"
                               ,graphFromNews(action._0)]
                              ,["newsItems",action._0]],
              model);
            case "NoOp": return model;
            case "Reset":
            return _U.replace([["phrase"
                               ,_L.fromArray([$Markov.startToken])]],
              model);}
         _U.badCase($moduleName,
         "between lines 24 and 30");
      }();
   });
   var newsStories = A2($Signal.map,
   $Http.mapResult($Json$Decode.decodeString($Schema.decodeNewsItems)),
   $Http.sendGet($Signal.constant("https://hn.algolia.com/api/v1/search_by_date?tags=story&hitsPerPage=200")));
   var model = A3($Signal.foldp,
   step,
   initialModel,
   $Signal.mergeMany(_L.fromArray([uiMailbox.signal
                                  ,A2($Signal._op["<~"],
                                  $Schema.LoadNews,
                                  newsStories)])));
   var main = A2($Signal._op["<~"],
   $UI.rootView(uiMailbox.address),
   model);
...
Elm.Markov.make = function (_elm) {
   ...
   if (_elm.Markov.values)
   return _elm.Markov.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
            ...
            return function () {
                 var updater = function (m) {
                    return function () {
                       switch (m.ctor)
                       {case "Just":
                          return $Maybe.Just(A2($Set.insert,
                            _v0._1,
                            m._0));
                          case "Nothing":
                          return $Maybe.Just($Set.singleton(_v0._1));}
                       _U.badCase($moduleName,
   ...
   var toSentence = function (s) {
      return A2($Basics._op["++"],
      _L.fromArray([startToken]),
      A2($Basics._op["++"],
      $String.words(s),
      _L.fromArray([endToken])));
   };
   var addChain = F2(function (s,
   g) {
      return function () {
   ...
   var createGraph = A2($List.foldl,
   addChain,
   initialGraph);
   _elm.Markov.values = {_op: _op
...
Elm.Schema.make = function (_elm) {
   ...
   var ChooseToken = function (a) {
      return {ctor: "ChooseToken"
             ,_0: a};
   };
   var LoadNews = function (a) {
      return {ctor: "LoadNews"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   ...
   var NewsItem = function (a) {
      return {_: {},title: a};
   };
   var decodeNewsItem = A2($Json$Decode.map,
   NewsItem,
   A2($Json$Decode._op[":="],
   "title",
   $Json$Decode.string));
   var decodeNewsItems = A2($Json$Decode.at,
   _L.fromArray(["hits"]),
   $Json$Decode.list(decodeNewsItem));
   _elm.Schema.values = {_op: _op
                        ,NewsItem: NewsItem
                        ,decodeNewsItem: decodeNewsItem
                        ,decodeNewsItems: decodeNewsItems
                        ,Model: Model
                        ,NoOp: NoOp
                        ,LoadNews: LoadNews
...
Elm.UI.make = function (_elm) {
   ...
   token) {
      ...
      _L.fromArray([$Html$Attributes.$class("btn btn-info")
                   ...
                   uiChannel,
                   $Schema.ChooseToken(token))]),
      _L.fromArray([$Html.text(token)]));
   });
   var tokenButtons = F2(function (uiChannel,
   tokens) {
      return A2($Html.div,
      _L.fromArray([]),
      A2($List.map,
   ...
   var body = F2(function (uiChannel,
   model) {
      return function () {
         ...
         _L.fromArray([A2($Html.div,
                      ...
                      _L.fromArray([A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-8 col-sm-offset-2")]),
                      _L.fromArray([A2($Html.blockquote,
                      _L.fromArray([]),
                      ...
                      _L.fromArray([$Html$Attributes.$class("row")]),
                      _L.fromArray([A2($Html.div,
                                   _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-2 well")]),
                                   _L.fromArray([$Html.text("Click any button to choose the next word.")]))
                                   ,A2($Html.div,
                                   _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-8")]),
                                   _L.fromArray([function () {
                                      switch (nextTokens.ctor)
                                      {case "Just":
                                         return A2(tokenButtons,
                                           uiChannel,
                                           $Set.toList(nextTokens._0));
                                         case "Nothing":
                                         return A2($Html.div,
                                           _L.fromArray([]),
                                           _L.fromArray([]));}
                      ...
                      _L.fromArray([$Html$Attributes.$class("col-xs-12 col-sm-8 col-sm-offset-2")]),
                      _L.fromArray([function () {
                         var _v2 = model.newsItems;
                         switch (_v2.ctor)
                         {case "Success":
                            return A2($Html.div,
                              _L.fromArray([]),
                              A2($List.map,itemView,_v2._0));}
                         return loading;
                      }()]))]))]));
      }();
   });
