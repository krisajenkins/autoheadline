...
body uiChannel model =
  div []
      ...
      ,case model.newsItems of
         Just (Ok items) -> newsBody uiChannel model.phrase items
...
newsBody uiChannel currentPhrase newsItems =
  let currentToken = Maybe.withDefault Markov.startToken (List.head (List.reverse currentPhrase))
      graph = graphFromNews newsItems
      nextTokens = Dict.get currentToken graph
  in div []
         [div [class "row"]
